  ...
  return (
    <div className="space-y-6">
      <section className="space-y-3 rounded-2xl border border-border/40 bg-card/35 p-5">
        ...
        <div>
          <h1 className="text-2xl font-display font-semibold text-foreground sm:text-3xl">{report.title}</h1>
        </div>
  ...
  return (
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      {report.stats.map((stat) => (
        <div key={`${stat.label}-${stat.value}`} className="rounded-xl border border-border/35 bg-card/50 p-4">
      ...
      <div className="h-[220px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={report.pnl_series}>
            ...
            <XAxis dataKey="x" tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} />
            <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} />
            <Tooltip contentStyle={tooltipStyle} />
            <Area type="monotone" dataKey="pnl" stroke={lime} fill="url(#pnl-grad)" strokeWidth={2} dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </ChartCard>
        ...
        <div className="h-[220px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chart.series}>
              ...
              <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Area type="monotone" dataKey="value" stroke={red} fill="url(#drawdown-grad)" strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </ChartCard>
        ...
        <div className="h-[220px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chart.series}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(0,0%,15%)" />
              <XAxis dataKey="x" tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} />
              <YAxis tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} domain={[0, 100]} />
              <Tooltip contentStyle={tooltipStyle} />
              <Line type="monotone" dataKey="primary" stroke={lime} strokeWidth={2} dot={false} name="Primary" />
              <Line type="monotone" dataKey="secondary" stroke={orange} strokeWidth={2} dot={false} name="Secondary" strokeDasharray="4 4" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </ChartCard>
        ...
        <div className="h-[220px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chart.series} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(0,0%,15%)" />
              <XAxis type="number" tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} />
              <YAxis type="category" dataKey="label" tick={{ fill: "hsl(0,0%,40%)", fontSize: 10 }} axisLine={false} width={90} />
              <Tooltip contentStyle={tooltipStyle} />
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                {chart.series.map((point) => (
                  <Cell
                    key={point.label}
                    fill={point.value < 0 ? lime : point.value > 0 ? red : "hsl(0,0%,35%)"}
                    fillOpacity={0.75}
                  />
                ))}
              </Bar>
        ...
        <div className="space-y-3">
          {chart.series.map((point) => (
            <div key={point.label} className="flex items-center gap-3">
              <span className="w-24 shrink-0 font-mono text-xs text-muted-foreground">{point.label}</span>
              <div className="relative h-6 flex-1 overflow-hidden rounded-full bg-muted/30">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-primary/60 to-primary"
                  style={{ width: `${Math.max(0, Math.min(100, point.value * 100))}%` }}
                />
                <span className="absolute inset-0 flex items-center justify-center text-[10px] font-medium text-foreground">
                  rho = {point.value}
                </span>
              </div>
              <span className="w-14 text-right text-[10px] text-muted-foreground">z = {point.secondary}</span>
            </div>
          ))}
        </div>
      </ChartCard>
    ...
    <section className="overflow-hidden rounded-xl border border-border/35 bg-card/45">
      ...
      <div className="divide-y divide-border/15">
        {report.key_metrics.map((metric) => (
          <div key={`${metric.metric}-${metric.value}`} className="flex items-center justify-between gap-4 px-4 py-3">
            <span className="text-sm text-muted-foreground">{metric.metric}</span>
            <div className="text-right">
              <div className={`text-sm font-medium ${metric.highlight ? (metric.positive ? "text-primary" : "text-destructive") : "text-foreground"}`}>
                {metric.value}
              </div>
              {metric.sub && <div className="text-[10px] text-muted-foreground/70">{metric.sub}</div>}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-border/35 bg-card/45">
      <div className="flex items-center gap-2 border-b border-border/25 bg-card/75 px-4 py-3">
        ...
        <span className="text-xs font-medium text-foreground">Monte Carlo Robustness</span>
      </div>
        ...
        <table className="w-full min-w-[520px] text-sm">
          <thead>
            <tr className="border-b border-border/25">
              ...
              <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Mean</th>
              <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Std</th>
          ...
          <tbody className="divide-y divide-border/15">
            {report.monte_carlo_table.map((row) => (
              <tr key={row.metric}>
                ...
                <td className="px-3 py-2 text-right font-medium text-foreground">{row.mean}</td>
                <td className="px-3 py-2 text-right text-muted-foreground">{row.std}</td>
                <td className="px-3 py-2 text-right text-muted-foreground">{row.min}</td>
                <td className="px-3 py-2 text-right text-muted-foreground">{row.max}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-border/35 bg-card/45">
      <div className="flex items-center gap-2 border-b border-border/25 bg-card/75 px-4 py-3">
        ...
        <span className="text-xs font-medium text-foreground">Parameter Sensitivity</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] text-sm">
          <thead>
            <tr className="border-b border-border/25">
              ...
              <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Win%</th>
              <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Avg PnL</th>
              <th className="px-3 py-2 text-right text-xs font-medium text-muted-foreground">Total PnL</th>
          ...
          <tbody className="divide-y divide-border/15">
            {report.sensitivity.map((row) => (
              <tr key={row.scenario}>
                <td className="px-4 py-2.5 text-muted-foreground">{row.scenario}</td>
                <td className="px-3 py-2.5 text-right text-foreground">{row.win}</td>
                <td className={`px-3 py-2.5 text-right font-medium ${row.avg_pnl.startsWith("-") ? "text-destructive" : "text-primary"}`}>{row.avg_pnl}</td>
                <td className={`px-3 py-2.5 text-right font-medium ${row.total_pnl.startsWith("-") ? "text-destructive" : "text-primary"}`}>{row.total_pnl}</td>
                <td className="px-2 py-2.5 text-center">
                  {row.tag === "best" && <Sparkles className="inline h-3.5 w-3.5 text-yellow-400" />}
                  {row.tag === "danger" && <XCircle className="inline h-3.5 w-3.5 text-destructive" />}
                  {row.tag === "baseline" && <Target className="inline h-3.5 w-3.5 text-muted-foreground" />}
                </td>
              </tr>
            ))}
          </tbody>
...
function WhyItWorks({ report }: { report: ThesisReportV1 }) {
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-base font-display font-semibold text-foreground">Why It Works</h2>
      <div className="grid gap-3 sm:grid-cols-3">
        {report.reasons.map((reason, index) => {
          const Icon = icons[index % icons.length];
          return (
            <article key={reason.title} className="rounded-xl border border-border/35 bg-card/45 p-4">
              ...
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{reason.desc}</p>
            </article>
          );
        })}
      </div>
...
function EconomicsCards({ report }: { report: ThesisReportV1 }) {
  return (
    <section className="space-y-3">
      <h2 className="text-base font-display font-semibold text-foreground">Economics</h2>
      <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
        {report.economics.map((entry) => (
          <article key={entry.label} className="rounded-xl border border-border/35 bg-card/45 p-4 text-center">
            <div className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{entry.label}</div>
            <div className="mt-2 text-lg font-semibold text-primary">{entry.pnl}</div>
            <div className="mt-1 text-xs text-muted-foreground">{entry.size}</div>
...
function VerdictCard({ report }: { report: ThesisReportV1 }) {
  return (
    <section className="rounded-xl border border-primary/30 bg-primary/5 p-4">
      ...
      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{report.verdict}</p>
    </section>
  );
}
