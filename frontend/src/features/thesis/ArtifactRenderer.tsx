...
function ChartArtifactRenderer({
  artifact,
}: {
  artifact: Extract<ThesisArtifact, { type: 'chart_v1' }>['data'];
}) {
  return (
      ...
      <header className="space-y-2">
        ...
        <div className={proseClasses}>
          <MarkdownChartRenderer
            markdown={artifact.summary_markdown}
            variant="compact"
          />
        </div>
      </header>
...
function RechartsBarArtifactRenderer({
  ...
  artifact: Extract<
    Extract<ThesisArtifact, { type: 'chart_v1' }>['data'],
    { renderer: 'recharts'; kind: 'bar' }
  >;
}) {
