...
vi.mock('@galpha-ai/better-markdown/react', () => ({
  MarkdownChartRenderer: ({ markdown }: { markdown: string }) => (
...
describe('ReplayPage', () => {
  ...
  it('renders structured assistant artifacts in replay mode', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          messages: [
    ...
    expect(await screen.findByText('Replay')).toBeInTheDocument();
    expect(await screen.findByTestId('vendor-markdown')).toHaveTextContent(
      '<<<CHART_V1>>>',
    );
  });
  ...
  it('renders a replay-specific error state when the API request fails', async () => {
    fetchMock.mockResolvedValue(
      new Response(JSON.stringify({ message: 'Replay not found' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' },
      }),
    );
