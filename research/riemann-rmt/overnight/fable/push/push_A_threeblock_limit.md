...
## Repair pass (6 September 2026, after `refute_A_limit_theorem.md`)

The refuter's verdict was "minor-issues": every [P] statement survived its line-by-line audit and independent recomputation, and errors were found only in [C] claims and in one derivation. Each issue was re-derived independently here (`repair_A_verify.py`, sections named below; logs `repair_A_verify_*.log`); all of the refuter's substantive findings are **valid** and have been fixed in place. Corrections are marked "*repair pass*" where they occur. Two additional slips not raised by the refuter were found and fixed (S1, S2).

| # | issue (refuter's tag) | verdict | fix |
|---|---|---|---|
| E1 | the "9-block" local model u(u²−4π²)(u²−16π²)cos(u/2) with τ* = 2.0689 is the **11-block**; the 9-block is (u²−π²)(u²−9π²)sin(u/2) with τ* = **2.0573579730** | valid | parity rule stated in §7.1; constants corrected in §0(5), §7.4, §8 (`models`: 2, 2, 2.03812605359, 2.05735797304, 2.06889355965 for the 3-…11-block); Prop. 7.3 constant for the true 9-block added, **−5.6436**, matching the finite-N data (−5.98, −5.73, −5.66 at N=33, 65, 129; `fold`, `blocks`) |
| E2 | §6.4 cusp derivation: tilt term has the wrong sign (−πv²/4 → **+πv²/4**), and an O(ε²) constant C_2(2) = **−4π/3** was omitted; the two errors cancel in C², so 2(π²/4)^{1/3} = 2.70257 was right for the wrong reason; the deliverable's own N=256 datum (2.695, from `heat_depth.py`) was a 4·10^{−9} solver error and the table did not support the claim | valid | Lemma 3.3 proof and §6.4 rewritten with the correct series, C_2, the normal form with the u² term, and the observation that the two ε^{7/3} corrections cancel identically (which predicts the observed N^{−4/3} approach); the middle-root trajectory corrected; table replaced by 2-D-root values to 15 digits, N = 16…2048, first collision verified (`tilt`, `asym`): residual 2.7750, 2.7301, 2.7132, 2.7067, 2.7042, 2.7032, 2.7028 at N = 32…2048 → 2.70257 |
| E3a | Prop. 6.1's proviso "true for all N ≥ 4 by Theorem 5.4" is an overreach (Theorem 5.4 has no explicit N_0) | valid | **repaired, not downgraded**: new **Lemma 6.0 [P]** proves a_N(3) < 0 for all N ≥ 13 (block4odd) / N ≥ 14 (block33) by an explicit Gaussian-moment bound (`prop61`), so the first zero of F_N is < 3/N² < −log cos(π/N) for every N; Prop. 6.1 now holds for all N |
| E3b | hypothesis (H) of Theorem 7.2 ("count equals n−2") is false for every symmetric model (mirror folds drop the count by 4) | valid | (H) restated as "≤ n−2", which is all the proof uses; the automatic-(H) paragraph adjusted |
| E3c | §6.3: the generic-angle shift is of order λ^{2/3}, not λ² | valid | corrected, with the fold computation (δ = (32λ²)^{1/3}ε(1+o(1))) |
| E3d | enumeration agreement is 6·10^{−7} at N=11, not "10^{−8}" | valid | caption corrected; cause identified: `np.roots` resolves the triple-zero cluster of the stored canonical representative only to ε_mach^{1/3}, so the enumeration's 10^{−7} threshold fires early — re-running its `depth_bisect` on that representative reproduces 1.9888690636 exactly (`enumtol`) |
| E3e | the block5 O(N^{−2}) failure is `run_families_A.py`'s, not `run_families.py`'s | valid | §0(6), §7.4 corrected (`famA.log` vs `families2.log`) |
| E3f | `heat_depth_robust.py` is 5·10^{−6} (7-block) / 5·10^{−4} (9-block) off at N=129; the 7-block corrections 4.28, 4.19, 4.25 were too noisy for "confirmed"; `midpoint_models.py` prints a spurious two3 line; `threeblock_exact.py` prints a spurious rel.err | valid | §7.4 finite-N table replaced by exact-coefficient 2-D-root values with mp zero-count verification (`blocks`), which agree with the refuter's fold solver to 12 digits and give −4.2795, −4.1856, −4.1620, −4.1560 (7-block, N = 33…257) → −4.154; the two script artefacts are recorded in §7.4 (scripts left unmodified, as instructed; the numbers in the text were right) |
| S1 | (self-found) Prop. 3.4's outer tail bound "≤ N^{−2} for N ≥ 8" is false at N=8 (it is 3.2·10³); true for N ≥ 21 | — | corrected; the proposition is unaffected (finitely many N are absorbed into C_1) (`tail`) |
| S2 | (self-found) the block4even N=192 datum 1.999961627887 from `heat_depth.py` was also 4.6·10^{−9} high (exact 1.99996162323976) | — | replaced (`asym`) |

Nothing is withdrawn: no [P] claim was refuted, and the two affected [C] claims (the 9-block constant; the N^{−8/3} coefficient) are now correct and better supported than before.

### Refuter responses

The refuter was right on every point of substance; there is nothing to rebut. Three marginal remarks on the refuter's report, for the record:

1. *Direction of the `heat_depth.py` error.* §1 of the refuter's report says the N=256 block4even value "1.999978634911 is 4.4·10^{−9} **below** the true 1.999978630550"; it is *above* (its §2.2 correctly calls it a solver error). Both `heat_depth.py` values at N=192 and 256 are high by 4–5·10^{−9}, which is why the deliverable's scaled residual ran *through* the predicted limit.
2. *Prop. 6.1.* The refuter's proposed status ("[P] for N ≥ N_0, [C] for each tabulated N") was weaker than necessary: the proviso can be proved for all N ≥ 13/14 by an explicit bound (Lemma 6.0), so the proposition is restored to [P] for every N rather than downgraded.
3. *Convergence rate in §2.2 of the refuter's report.* The refuter observed the scaled residual decaying by a factor 2.53 per doubling and called it N^{−4/3} without explanation; the corrected §6.4 explains it — the two ε^{7/3} corrections at the fold cancel identically — so the rate is now a derived consequence, not just an observation (`asym`: successive residual ratios 2.35, 2.43, 2.48, 2.51 for N = 128…2048, vs 2^{4/3} = 2.52).
...
5. **Midpoint-insertion principle, Proposition 7.1 [P].** For k midpoints inserted with polynomial p and lattice L∈{cos(u/2), sin(u/2)},
   ...
   with e^{τ∂²}p = Σ_k τ^k p^{(2k)}/k! — a closed form for every such family. Consequences: the **5-block** (gaps 1,1,1,1) and the **double 3-block** (gaps 1,1,2,1,1) both have local constant τ* = **2**, with the double zero at u=π resp. u=2π occurring *exactly* at τ=2 [P]; that no double zero occurs earlier is reduced to a one-variable system and checked [C]. For the (2k+1)-block [1]^{2k} the local constants are τ* = 2, 2, **2.03812605359**, **2.05735797304**, **2.06889355965** for k = 1…5 (3-, 5-, 7-, 9-, 11-block) [C] (*repair pass:* the original text called the k=5 polynomial u(u²−4π²)(u²−16π²)cos(u/2) the "9-block" and quoted 2.0689 for it; the 9-block is (u²−π²)(u²−9π²)sin(u/2), §7.1): longer blocks collide *later*, consistent with the enumeration's maximisers at N=11,12 being long 1-runs, so the 3-block is not the maximiser of N²D beyond N=10. First-order corrections N²(τ_N−τ*) → −8/3, −8/3, −4.154, −5.64 for the 5-block, double 3-block, 7-block and 9-block (Prop. 7.3 [P] formula, values [C]) are confirmed by exact-coefficient fold solvers at N up to 257 [C] (§7.4).

6. **A failure mode of `heat_depth.py` [C].** Its `depth_pair` discards the tracked extremum as soon as it leaves the *initial* interval (θ_i,θ_{i+1}); for defects whose colliding roots drift (7-block outer gap, and the 5-block / double-3-block gaps, whose merge point is an *endpoint* of the initial gap) this returns a spurious early time (7-block: 1.843 instead of 2.038). `heat_depth_robust.py` fixes this by bracketing with the current zeros; it reproduces the exact 3-block values to 10^{−9}, but is accurate only to ≈5·10^{−6} (7-block) and ≈5·10^{−4} (9-block) at N=129, where three roots crowd the fold (§7.4). *Repair pass:* the script whose `block5` column is wrong at the O(N^{−2}) level (N²(2−τ_N) ≈ 9.3 instead of 8/3) is `run_families_A.py` (which scans pair 3); `run_families.py`'s `block5` (pairs 0,1,2,N−1) happened to be right (2.68, 2.70 at N=256, 384); `block7` is wrong at O(1) in both.
...
*Proof.* (B1),(B3): with a=v/2N (|a|≤1/2) and b=π/2N, block4odd is 2N(tan a) = v + v³/(12N²) + v⁵/(120N⁴)+…, a convergent Taylor series with positive coefficients, so the remainders are bounded by the next term times an absolute constant; the derivative is sec²a−1 = tan²a ≤ 2a². For block33, the numerator N sin(v/N)cos²b − v(cos²a−sin²b) equals (N sin(v/N)−v) − N sin(v/N) sin²b + v sin²a + v sin²b, each term being O((|v|³+|v|)/N²), and the denominator cos²a − sin²b ≥ cos²(1/2) − sin²(π/16) > 0.7; the derivative bound is the same computation one order down. block4even: φ_N − v = 2N[sin a cos b − a cos(a+b)]/cos(a+b), and the numerator sin a cos b − a cos(a+b) = cos b(sin a − a cos a) + a sin a sin b = O(a³ + a²b), with cos((v+π)/2N) ≥ cos(1/2+π/16) > 0.5; the a²b term produces the even contribution **+πv²/(4N²)** that is responsible for the asymmetry (§6.4). (*Repair pass:* the original text had −πv²/(4N²), a sign slip; sin a cos b/cos(a+b) = tan a/(1 − tan a tan b) = tan a(1 + ab + …), and numerically N²(φ_N(v) − v − v³/12N²) = +0.7854, +3.1416 at v = 1, 2, N = 1000. The full expansion is φ_N = v + (v³/12 + πv²/4)/N² + (v⁵/120 + πv⁴/24 + π²v³/16 + π³v²/48)/N⁴ + O(N^{−6}), `repair_A_verify.py tilt`.) In every case the Taylor remainders are those of sin, cos, tan on |a|≤1/2 and are controlled by the next term; the symbolic expansions are printed by `threeblock_asymptotics.py` and `fold_constants.py`, and (B1),(B3) are confirmed numerically to the stated orders in `local_convergence_check.py` [C].
...
*Proof.* Put g := q_0^N − q_0 where q_0(v) = v cos(v/2); then g(v) = cos(v/2)(φ_N(v)−v), and q_τ^N − q_τ = G_τ * g, ∂_u(q_τ^N − q_τ) = G_τ * g′ (differentiate q_0^N and q_0 rather than the kernel, so that no τ^{−1/2} appears). By (B1), |g(v)|+|g′(v)| ≤ 2C_0(1+|v|³)/N² for |v| ≤ N; by (B2), |g|+|g′| ≤ 2N³+2(1+|v|) everywhere. Split the convolution at |v| = N. Inner part: ≤ 2C_0 N^{−2} E[1+|u+√(2τ)Z|³] ≤ 2C_0 N^{−2}(1 + 8(2π)³ + 8·6^{3/2}E|Z|³) for |u|≤2π, τ≤3, Z standard Gaussian. Outer part: with V := u+√(2τ)Z, ≤ E[(2N³+2+2|V|)·1_{|V|≥N}] ≤ (2N³+2)P(|V|≥N) + 2E[|V|1_{|V|≥N}] ≤ 4N³·2e^{−(N−2π)²/12} for τ≤3, |u|≤2π, N≥8 (P(|V|≥N) ≤ 2e^{−(N−2π)²/12}, E[|V|1_{|V|≥N}] ≤ (2π+√6)·2e^{−(N−2π)²/12}, and 2N³ dominates the remaining constants). This is ≤ N^{−2} for N ≥ 21 (*repair pass:* the original said "for N ≥ 8", which is false — at N=8 the right side is 3.2·10³; since the outer part is finite for each N, the finitely many N in [8,20] are absorbed into C_1, and the statement of the proposition is unchanged). ∎
...
**Lemma 6.0 (the first zero of F_N lies in (0, 3/N²)).** [P] Let a_N(τ) := ∂_u q_τ^N(0) = e^{−τ/4}F_N(τ/N²)/F_N(0) (block4odd; block33 with the same normalisation), so that a_N(0) = φ_N′(0) = 1. Then a_N(3) < 0 for every odd N ≥ 13 (block4odd) and every even N ≥ 14 (block33); hence F_N has a zero in (0, 3/N²), and 3/N² < π²/(2N²) ≤ −log cos(π/N).

*Proof.* By Lemma 3.1, 2τ·a_N(τ) = E[V q_0^N(V)] with V ~ N(0,2τ); write q_0^N(v) = cos(v/2)φ_N(v) and split off the limit: 6a_N(3) = E[V²cos(V/2)] + E[V cos(V/2)(φ_N(V)−V)], and E[V²cos(V/2)] = (2τ−τ²)e^{−τ/4}|_{τ=3} = −3e^{−3/4} = −1.4171. *block4odd:* on |a| ≤ 1/2, (tan a − a)/a³ is increasing with value 0.3704 at a=1/2, so |φ_N(v)−v| = 2N|tan(v/2N) − v/2N| ≤ |v|³/(9N²) for |v| ≤ N, whence |E[V cos(V/2)(φ_N(V)−V); |V|≤N]| ≤ E[V⁴]/(9N²) = 12/N²; on |V| > N, |V cos(V/2)(φ_N(V)−V)| ≤ |V|(2N²+|V|) by (B2) (|q_0^N| ≤ 2N² for this family), and E[|V|(2N²+|V|); |V|>N] ≤ 5·10^{−4} at N=13, decreasing in N. So 6a_N(3) ≤ −1.4171 + 12/169 + 0.0005 < −1.34 < 0 for N=13, and the bound only improves with N. *block33:* φ_N(v) − v = {(N sin(v/N) − v)(1−sin²b) + v sin²a}/(cos²a − sin²b), so |φ_N(v)−v| ≤ (1/6+1/4)|v|³/(N²·0.7577) ≤ 0.55|v|³/N² for |v| ≤ N, N ≥ 14 (cos²a − sin²b ≥ cos²(1/2) − sin²(π/28)); the inner part is ≤ 0.55·108/N² = 59.4/N² ≤ 0.303 at N=14, and the tail with |q_0^N| ≤ N³ is ≤ 5·10^{−4}; so 6a_N(3) ≤ −1.4171 + 0.304 < −1.11 < 0. Continuity of a_N on [0,3] gives the zero. The last inequality is Lemma 2 of the theorem file at x = 2π/N. ∎

The exact values (`repair_A_verify.py prop61`): 6a_N(3) = −1.4492 (N=13), −1.4357 (17), −1.4203 (41) for block4odd and −1.4458 (14), −1.4388 (16), −1.4204 (40) for block33, all within the stated bounds of −1.4171. For N ≤ 12 the first zero of F_N is computed directly (table below; all ≈ 1.94–1.99 < π²/2) [C].
...
for every N: the proof needs this minimum to be < −log cos(π/N), which is Lemma 6.0 for N ≥ 13 (block4odd) / N ≥ 14 (block33) and a direct evaluation for the smaller N. (*Repair pass:* the original proviso read "true for all N ≥ 4 by Theorem 5.4 and, for N ≤ 12, by the enumeration"; as the refuter pointed out, Theorem 5.4 is asymptotic with an unspecified N_0 and cannot certify the proviso for 13 ≤ N < N_0. Lemma 6.0 closes this gap with an explicit bound.)
...
**Verification [C]** (`threeblock_exact.py`, parts (3),(3b)): first zero of F_N versus the complete enumerations (`acue_depth_N*.npz`) and versus `heat_depth.py`. The enumeration agrees to 1–9·10^{−8} for N ≤ 10 but is 6.1·10^{−7} low at N=11 (*repair pass*, corrected from "agreement 10^{−8}"): the enumeration detects the collision by `np.roots` leaving the unit circle by more than 10^{−7}, and at a *triple* zero (the collision of these symmetric families) the three-root cluster is resolved only to ε_mach^{1/3}, so the threshold fires early — for the stored canonical representative (sites 0,1,2,4,6,8,10,14,16,18,20) the indicator is already 3·10^{−8} at s = D(1−10^{−6}) and 3·10^{−6} at s = D(1−10^{−8}); re-running the enumeration's `depth_bisect` on that representative reproduces its 1.9888690636 exactly (`repair_A_verify.py enumtol`). The exact-F_N values are the accurate ones.
...
φ_N − v starts at order N^{−2} because the compensation sits within O(1/N) of the antipode: the removed/added far roots contribute a factor ∏ sin((x−θ_r)/2)^{∓1} = cos((u−ρπ)/2N)^{∓1}·const with ρ=O(1), whose expansion 1 ∓ (u²−2ρπu)/(8N²)+… has no u/N term. For a compensation at a generic angle α the factor is ρ(0)(1 + λu/N + …) with λ = ½cot(α/2)-type constants, and then q_0^N = u cos(u/2)(1+λu/N)+…; the even perturbation (λ/N)·e^{τ∂²}[u²cos(u/2)] = (λ/N)e^{−τ/4}[(u²+2τ−τ²)cos(u/2) − 2τu sin(u/2)] vanishes at u=0 when τ=2 (because E[V²cos(V/2)] ∝ H_2(√τ/2) ∝ τ−2), so the cusp analysis of §6.4 shows the shift is still O(N^{−2}): with C = 2λσ/N and A = σ/2 − (2/3)ε the fold condition A³ = (9/16)C² gives σ_c − (4/3)ε = (32λ²)^{1/3}ε(1+o(1)) for small λ — *of order λ^{2/3}*, non-analytic in λ as a cusp should be (*repair pass:* the original said "of order λ²"; the u²-term and the σ-dependence of C cancel at the fold exactly as in §6.4, so this is the leading behaviour). A generic compensation changes the −4/3 but not the order. [P for the vanishing; the resulting constant for a given α is O: not computed.]

### 6.4 The asymmetric family block4even: a cusp and the N^{−8/3} term [C, with derivation]

*Repair pass (E2).* The derivation below is rewritten: the original had the tilt with the wrong sign (−πv²/4) and omitted an O(ε²) constant, C_2(2) = −4π/3, which at the fold is of the same order as the term kept; the two errors cancelled exactly in C², so the coefficient 2(π²/4)^{1/3} = 2.70257 was right for the wrong reason (refuter, confirmed symbolically and numerically in `repair_A_verify.py tilt`). The corrected bookkeeping also predicts the N^{−4/3} convergence rate of the scaled residual that the data show.

For block4even, φ_N(v) = v + (v³/12 + πv²/4)/N² + (v⁵/120 + πv⁴/24 + π²v³/16 + π³v²/48)/N⁴ + …, so q_τ^N = q_τ + N^{−2}(r_τ + r̃_τ) + N^{−4}(…) with the odd part r_τ as before and the even part r̃_τ = (π/4)e^{τ∂²}[v² cos(v/2)] = (π/4)e^{−τ/4}[(u²+2τ−τ²)cos(u/2) − 2τu sin(u/2)]. Near (u,τ)=(0,2) write σ = 2−τ, ε=N^{−2}. The constant term is, by E[V^{2n}cos(V/2)] = (−1)^n(d/dt)^{2n}e^{−τt²}|_{t=1/2} applied to the even terms πv²/4 (order ε) and πv⁴/24 + π³v²/48 (order ε²),

  e^{τ/4} q_τ^N(0) = ε·(π/4)τ(2−τ) + ε²·C_2(τ) + O(ε³),  C_2(τ) = −(π/48)τ(−2τ³+24τ²−24τ+π²τ−2π²),  C_2(2) = **−4π/3**,

so C := e^{τ/4}q_τ^N(0) = (π/2)σε − (4π/3)ε² + O(σ²ε, σε², ε³) — positive for σ > (8/3)ε, negative below. The local normal form is

  e^{τ/4} q_τ^N(u) = C + A u + B u² − u³/12 + O(u⁴),  A = σ/2 − (2/3)ε + O(εσ, ε², σ²),  B = −(π/4)ε + O(εσ, ε²)

(B is the u² coefficient of e^{τ/4}r̃_τ at τ=2). A cubic C + Au + Bu² − u³/12 has a real double root when, to leading order, C + (4/3)A^{3/2} + 4AB = 0 (expand about the B=0 double root u = 2√A). Put σ = (4/3)ε + δ, so A = δ/2 and C = −(2π/3)ε² + (π/2)εδ. The two corrections of relative order ε^{1/3} — the u² term 4AB = −πεA and the σ-dependence of C, (π/2)εδ = +πεA — **cancel identically** (this happens for every even perturbation of the form c·u²cos(u/2), whose constant and u² coefficients at τ=2 are 2c and −c). What remains is exactly the leading balance (4/3)A^{3/2} = (2π/3)ε², i.e. A³ = (9/16)C² with C = −(2π/3)ε²,

  A_c = (π²/4)^{1/3} ε^{4/3} = 1.35128 ε^{4/3},  δ = 2A_c,  τ_N = 2 − (4/3)N^{−2} − 2(π²/4)^{1/3} N^{−8/3} + O(N^{−10/3}),  2(π²/4)^{1/3} = **2.70257**,

and the fold happens *before* A reaches 0 (A_c > 0). Since the leading relative corrections cancel, the next term is of relative order ε^{2/3}, i.e. the scaled residual (N²(2−τ_N) − 4/3)N^{2/3} − 2.70257 should decay like N^{−4/3}. Two qualitative features of the original text were also wrong: the middle zero of q_τ^N first sits at u < 0 (C > 0 for σ > (8/3)ε), crosses u=0 at σ ≈ (8/3)ε and only then merges with the root coming from +π at u* = 2√A_c > 0; at N=64 the exact N²q_τ^N(0) is +0.008895, +0.000332, −0.000145, −0.000270 at τ = 1.99, 1.999, 1.9995, τ_N (two-term prediction +0.008882, +0.000332, −0.000144, −0.000270; the original formula −(π/2)e^{−τ/4}σ gives −0.00955 at τ=1.99, wrong sign), with zeros at u = −0.2416, −0.0007, +0.2400 (τ=1.99) and −0.0191, +0.0070, +0.0098 just before the merge at u* = +0.00832.
...
* **7-block** [1]^6: zeros of u(u²−4π²)·cos(u/2); **9-block** [1]^8: (u²−π²)(u²−9π²)·sin(u/2); **11-block** [1]^{10}: u(u²−4π²)(u²−16π²)·cos(u/2); **[1,1,2,2,1,1]**: (u²−9π²)·sin(u/2).

*Parity rule.* For the (2k+1)-block centred at u=0 the occupied sites are 0, ±π, …, ±kπ and the lattice continues at ±(k+2)π, ±(k+4)π, …; so the lattice has the parity of k: L = sin(u/2) and p = ∏_{j odd ≤ k−1}(u²−j²π²) for k even, L = cos(u/2) and p = u∏_{j even, 2≤j≤k−1}(u²−j²π²) for k odd. (*Repair pass:* the original listed u(u²−4π²)(u²−16π²)cos(u/2) as the "9-block"; its zeros 0, ±π, …, ±5π, ±7π, … are eleven consecutive sites, i.e. the 11-block. The refuter caught this.)
...
**Theorem 7.2.** Let a family of N-point ACUE configurations consist of the alternating clock with k added midpoints at fixed sites within O(1) of 0 (polynomial p, lattice L) and k lattice roots removed within O(1) sites of the antipode, so that q_0^N = L·Φ_N with |Φ_N − p| ≤ C(1+|v|^{k+2})/N² on |v|≤N and polynomial global bounds (as in Lemma 3.3; this holds by the same computation, the far factors being ∏cos((u−ρ_iπ)/2N)^{∓1}). Let q_τ := e^{τ∂²}[pL] (Prop. 7.1) and let τ* be the first τ>0 at which q_τ has a real zero of multiplicity ≥2. Assume
 ...
 (H) τ* < π²/2, and there is R>0 with q_τ(±R) ≠ 0 for τ∈[0,τ*+1], such that the number of zeros of q_τ in [−R,R] is constant (=n, all simple) for τ<τ*, and is **≤ n−2** (all simple) for τ∈(τ*, τ*+δ].

Then N²D_N → τ*.

*Proof.* Identical to Theorem 5.4 with [−2π,2π] replaced by [−R,R], Lemma 4.1 by (H), and "the colliding pair is a block pair" by Lemma 1.1 (τ* < π²/2); the upper bound only uses that the count after τ* is smaller than n. ∎

(*Repair pass:* the original (H) demanded "equals n−2". For every model to which the theorem is applied below, q_τ is odd or even in u, so the fold at u* and its mirror image at −u* occur at the same τ* and the count drops by 4; as literally stated (H) was false for these families, although the proof never used more than "< n". The refuter caught this.)

*When (H) holds automatically.* If the first double zero (u*,τ*) is non-degenerate (q_uu(u*,τ*) ≠ 0), then since ∂_τ q = ∂_u² q the extremum value h(τ) = q_τ(x_ext(τ)) between the colliding pair satisfies h′(τ*) = q_uu(u*,τ*) ≠ 0: the pair crosses transversally and the count drops by exactly 2 at that fold (by 4 when a mirror-image fold occurs simultaneously) — the second half of (H). If instead the collision is a symmetric triple zero at 0 (odd families), (H) follows from a sign change of a(τ) = q_τ′(0). The first half of (H) — that no earlier double zero exists — is the part that must be verified for each p; for p=u it is Lemma 4.1.
...
**7-, 9- and 11-block.** The first double zero of u(u²−4π²)cos(u/2) is at u*=5.9643126848, τ*=**2.03812605359** (q_uu=14.478): the pair starting at (2π,3π) collides at u*<2π, both roots having drifted inward. The 9-block model (u²−π²)(u²−9π²)sin(u/2) has its first double zero at u* = 8.8860856041 = 2.82853π, τ* = **2.05735797304** (q_uu = 261.15; the pair from (3π,4π), zeros just before the merge at 0.898π, 1.783π, 2.820π, 2.837π, 5.666π, 7.771π), and the 11-block model u(u²−4π²)(u²−16π²)cos(u/2) at u* = 11.860625931 = 3.77535π, τ* = **2.06889355965** (pair from (4π,5π)) — `repair_A_verify.py models`, tracking every real zero on (0,(k+5)π) on a τ-grid of step 10^{−3} and refining by a 2-D Newton step; the refuter's independent tracker gives the same values. *Repair pass (E1):* the original text attributed 2.0689 and the polynomial u(u²−4π²)(u²−16π²)cos(u/2) to the "9-block"; that polynomial has eleven consecutive zeros −5π…5π and is the 11-block, and `model_nine.log` ("zeros just before 2.88, 5.74, 8.55, 11.86, 11.86", pair from (4π,5π)) describes the 11-block. The finite-N 9-block data (table below) converge to 2.05736 with an O(N^{−2}) correction whose constant (−5.64) is exactly the Prop. 7.3 value for the corrected model; against 2.0689 they would give N²(τ_N−2.0689) = −18.7, −54.6, −196, not O(1). So the local constants of the (2k+1)-blocks are 2, 2, 2.0381, 2.0574, 2.0689 for k=1…5: longer blocks collide later. This matches the complete enumerations: the maximiser of N²D at N=11 is [1,1,1,1,1,1,2,2,8,2,2] (a 7-block) with 1.9918, and at N=12 it is an 8-block with 2.00002 > 2 (`enum_N9_12.log`); the 3-block ceases to be the maximiser at N=11, and the supremum of the lattice constant ρ_∞ over families is strictly larger than 2 [C].
...
| 9-block (N odd) | u⁶/3 − 5π²u⁴/3 − 41π⁴u²/3 + 15π⁶ | 1473.844 | 261.152 | **−5.6436** |
| 11-block (N odd) | 5u⁷/12 − 5π²u⁵ − 40π⁴u³ + 640π⁶u/3 | −45077.48 | −6318.80 | **−7.1339** |

(The 9- and 11-block rows were added in the repair pass, `repair_A_verify.py fold`, with Φ_N = (2N)^k·∏_{added}sin/∏_{removed}cos exactly as for the 5- and 7-block; the ε⁰ and ε¹ coefficients of Φ_N are checked to be p and 0.)

Finite-N values [C]. *Repair pass:* the original table quoted `heat_depth_robust.py`, which is accurate to 10^{−9} for the 3-block but only to 6·10^{−9}, 6·10^{−8}, 5·10^{−6} (7-block, N=33, 65, 129) and 1·10^{−8}, 1·10^{−6}, 5·10^{−4} (9-block) — at the fold three roots sit within 0.6 of each other in u and its Newton bracket degrades; its 7-block corrections 4.28, 4.19, 4.25 were therefore too noisy to call the −4.154 "confirmed". The values below are first double zeros (Q_s = ∂_xQ_s = 0) computed by 2-D Newton in mpmath (40 + N/8 digits) from the *exact* coefficients P(z) = (z^N−1)∏_{j≤k}(z−e^{iπ(2j−1)/N})/(z+e^{iπ(2j−1)/N}) (polynomial division, remainder ≤ 10^{−40}), residual |Q|+|Q_x| ≤ 10^{−50}, with "first collision" verified by an mpmath zero count on the block window (2k+5 zeros until τ_N−10^{−4}, 2k+1 at τ_N+10^{−4}, no drop on a τ-grid of step 0.05 from τ=1) — `repair_A_verify.py blocks`, log `repair_A_verify_blocks_{3,4,5}.log`. The refuter's independent fold solver (`refute_A_limit_theorem.py blocks`, product coefficients at 60+N/4 digits) gives the same digits.

Old robust-solver values, for the record (`heat_depth_robust.py`, superseded):
...
**Sanity of the solvers [C].** `heat_depth_robust.py` reproduces the exact block33 values 1.994613077983 (N=16) and 1.998686923391 (N=32) to 1.4·10^{−9} and 2.3·10^{−9}, but see the accuracy caveat above for the 7- and 9-block. `heat_depth.py` gives for the 7-block 1.775 (N=16), 1.827 (32), 1.842 (128), 1.8428 (256), all wrong; the independent root-based bisection of `defect_families.py` gives 2.0177 (N=16) and 2.0301 (N=24), agreeing with the robust solver (2.0177, 2.0302), with the exact-coefficient solvers (2.017717 at N=16 by `mp.polyroots`, refuter) and with the local model. The failure is in `depth_pair`: the Newton iterate for the extremum is rejected when it leaves the *initial* gap (a,b)=(θ_i,θ_{i+1}); in the 7-block both roots of the outer gap drift by ≈0.3 (in u) before colliding, and in the 5-block the merge point is an *endpoint* of the initial gap (the pair (π,2π) meets at π). *Repair pass:* the script affected at the O(N^{−2}) level is `run_families_A.py block5` (which scans pairs 0…5 and picks the spurious pair 3, reporting N²(2−τ_N) ≈ 9.3 instead of 8/3, `famA.log`); `run_families.py block5` (pairs 0,1,2,N−1, `families2.log`) gives 2.68, 2.70 at N=256, 384, i.e. the right constant — the original text attributed the failure to the wrong script. `disloc`, `block4`, `block33` are unaffected. Two further script artefacts noted by the refuter (scripts left unmodified; the numbers in the text are right): `midpoint_models.py` prints "two3: first double zero at u*=0.0 tau*=1.95" because its triple-zero test q_u(0,τ)=0 fires trivially for an *even* model — the double 3-block's double zero is at (2π,2) as stated (§7.4, confirmed by the refuter's tracker and by the finite-N data); and `threeblock_exact.py` part (1) prints "rel.err Q0 = 2.64" for block4odd N=11, an artefact of its sign-ambiguity handling on a grid containing a pole-cancelled point — the coefficient check in the same part (10^{−14}) and the refuter's independent coefficient check (≤3·10^{−13}) are what matter.
...
| First zero of F_N lies in (0,3/N²) for all N ≥ 13 / 14 (Lemma 6.0, *added in the repair pass*) | **[P]** (explicit Gaussian-moment bound); N ≤ 12 by direct evaluation [C] |
| D_N = first zero of P_s′(1) for the symmetric families (Prop. 6.1) | **[P]** for every N (via Lemma 6.0; the original "for all N ≥ 4 by Theorem 5.4" was an overreach), verified to 10^{−8}/10^{−9} |
| τ_N = 2 − 4/(3N²) + c₂/N⁴ + O(N^{−6}), c₂ = −8/5, −8/5−π² (Prop. 6.2) | **[P]** (coefficient algebra by script; remainder bounds as in Lemma 3.3); verified to 7 digits, reproduced independently by the refuter to 15 digits |
| block4even: τ_N = 2 − 4/(3N²) − 2.70257·N^{−8/3} + O(N^{−10/3}) | **[C]**: coefficient 2(π²/4)^{1/3} confirmed to 4 digits (2.70267 at N=4096, refuter; 2.70282 at N=2048 here) with the predicted N^{−4/3} approach; the derivation is now correct (§6.4, repaired: tilt sign, the ε² constant, and the cancellation of the ε^{7/3} terms); making it [P] needs the O(N^{−4}) remainder in C² — routine but not written |
| Midpoint-insertion closed form (Prop. 7.1); limit theorem under (H) (Thm 7.2, with (H) corrected to "≤ n−2"); fold correction (Prop. 7.3) | **[P]** |
...
| 7-block τ*=2.0381260536, **9-block 2.0573579730**, 11-block 2.0688935597; N²(τ_N−τ*) → −8/3, −8/3, −4.154, −5.644 | **[C]**; the 9-block constant is *corrected* (the original 2.0689 was the 11-block's), and the finite-N corrections are now confirmed with exact-coefficient solvers (−4.156 at N=257, −5.66 at N=129) |
| 3-block is the maximiser of N²D over non-clock ACUE configurations | **false for N ≥ 11** [C]: longer blocks are deeper; the supremum over families exceeds 2 (7-block 2.038, 9-block 2.057, 11-block 2.069, enumeration argmax at N=12 is 2.00002 already). Determining sup_families lim N²D (half-block? ) is **[O]** |
| Compensation at a generic angle α: limit still 2, constant in N^{−2} depends on α | limit **[P]** by the same proof once a 1/N tilt is allowed in Lemma 3.3 (the tilt is an even perturbation and does not affect Theorem 5.4); the N^{−2} constant **[O]** (its cusp part is (32λ²)^{1/3}, of order λ^{2/3}, not λ²) |
| `heat_depth.py` failure mode; `heat_depth_robust.py` accuracy | failure mode **[C]** confirmed by three independent methods; the robust solver is 10^{−9}-accurate for the 3-block but only 5·10^{−6} (7-block) / 5·10^{−4} (9-block) at N=129 — superseded by exact-coefficient fold solvers for those families |
...
* `heat_depth_robust.py` — corrected solver; logs `robust2.log`, `robust3.log` (accurate for the 3-block only, see §7.4).
* `repair_A_verify.py` (*repair pass*) — independent re-derivations for every issue raised by the refuter, sections `models` (parity rule, first double zeros of the 3-…11-block models), `fold` (Prop. 7.3 constants incl. the 9- and 11-block), `tilt` (block4even series, C_2(2) = −4π/3, cusp bookkeeping, trajectory at N=64), `asym` (block4even τ_N to 15 digits, N ≤ 2048, with first-collision verification), `blocks` (7-, 9-, 11-block finite-N values from exact coefficients), `enumtol` (the N=11 enumeration discrepancy), `prop61` (Lemma 6.0's bound), `tail` (Prop. 3.4's tail constant); logs `repair_A_verify_<section>.log`.
* `refute_A_limit_theorem.py` (refuter) — the independent checks summarised in `refute_A_limit_theorem.md`; its `blocks`/`asym2` values agree with `repair_A_verify.py` to all printed digits.
