...
- `db.ts`: SQLite persistence via better-sqlite3. Owns the `messages`, `chats`, `sessions`, `agents`, `scheduled_tasks`, `task_run_logs`, and `router_state` tables and handles schema migrations from legacy formats. `messages` rows now also carry `ui_message_json`, a persisted canonical `UIMessage` projection used by web/replay reads when available.
...
12. **Canonical UI projection is persisted at the message row.** Each stored message may carry `ui_message_json`, a canonical `UIMessage` projection. Web/replay canonical reads prefer this projection over fallback derivation from raw `content`, which reduces read-path reconstruction logic and keeps persistence closer to AI SDK-native semantics.
