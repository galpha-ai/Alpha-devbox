...
- `db.ts`: SQLite persistence via better-sqlite3. Owns the `messages`, `chats`, `sessions`, `agents`, `scheduled_tasks`, `task_run_logs`, and `router_state` tables and handles schema migrations from legacy formats.
...
- `../frontend/`: Minimal browser frontend for the web channel. Uses a client-generated UUID sent as `X-User-Id`, talks to the existing `/api/devbox/*` endpoints, uses WebSocket for live status/output when available, and falls back to REST polling when the socket is unavailable.
