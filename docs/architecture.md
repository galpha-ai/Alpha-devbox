...
- `channels/web.ts`: Web adapter using plain HTTP. Serves the backend canonical `UIMessage[]` read endpoint for web chat, and exposes the AI SDK-compatible streaming endpoint (`POST /api/devbox/chat`) that maps controller `sendMessage` / `setTyping` callbacks directly into SSE UI message chunks. The live web write path accepts a **last-message-only** submit body (`{ id, message }`) from the AI SDK transport instead of full client history; prior canonical history is persisted server-side. Canonical reads lazily upgrade legacy rows by writing back the normalized `ui_message_json` projection when older rows are encountered for live conversations; replay reads stay read-only. Identity comes from the upstream Envoy proxy (`X-User-Id` header); in addition, every `/api/devbox/*` request must carry an `X-Access-Password` header matching the configured shared password (`WEB_ACCESS_PASSWORD` env var or `web.access_password` config, timing-safe compare). If no password is configured the channel fails closed and refuses all API requests with 503. Implements the `Channel` interface without DB polling or reply-stability heuristics; normalization stays server-side.
