...
describe('TelegramChannel', () => {
  ...
  describe('text message handling', () => {
    ...
    it('uses sender name as chat name for private chats', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:user:99001',
        expect.objectContaining({
          chat_jid: 'tg:user:99001',
        }),
      );
    });

    it('binds DM users via tg:user:* default registration', async () => {
      ...
      const opts = createTestOpts({
        registeredAgents: vi.fn(() => agents),
        registerAgentForChat: vi.fn((chatJid, agent) => {
          agents[chatJid] = agent;
        }),
      });
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.registerAgentForChat).toHaveBeenCalledWith(
        'tg:user:778899',
        expect.objectContaining({ agentName: 'main' }),
      );
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:user:778899',
        expect.objectContaining({ chat_jid: 'tg:user:778899' }),
      );
    });
  ...
  describe('sendMessage', () => {
    ...
    it('strips tg:user: prefix for DM JIDs', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:user:778899', 'DM message');
  ...
  describe('setTyping', () => {
    it('sends typing action when status is processing', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.setTyping('tg:100200300', 'processing');
    ...
    it('sends typing action for tg:user: JIDs', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.setTyping('tg:user:778899', 'processing');
    ...
    it('does nothing when status is not processing', async () => {
      ...
      await channel.setTyping('tg:100200300', 'success');
      await channel.setTyping('tg:100200300', 'error');
      await channel.setTyping('tg:100200300', 'idle');
    ...
    it('handles typing indicator failure gracefully', async () => {
      ...
      await expect(
        channel.setTyping('tg:100200300', 'processing'),
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe('bot commands', () => {
    ...
    it('/chatid shows chat type', async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith(
        expect.stringContaining('tg:user:555'),
        expect.any(Object),
      );
    });
