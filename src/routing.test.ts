...
import { getAvailableGroups, _setRegisteredAgents } from './index.js';
...
describe('JID ownership patterns', () => {
  ...
  it('Slack DM JID: starts with slack:D', () => {
    ...
    expect(jid.startsWith('slack:D')).toBe(true);
  });
});
...
describe('getAvailableGroups', () => {
  ...
  it('mixes Telegram and Slack chats ordered by activity', () => {
    storeChatMetadata(
      'tg:100',
      '2024-01-01T00:00:01.000Z',
      'Telegram',
    ...
    expect(groups[1].jid).toBe('tg:200');
    expect(groups[2].jid).toBe('tg:100');
  });
});
