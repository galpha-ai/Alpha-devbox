...
describe('SessionQueue', () => {
  ...
  it('does NOT preempt active container when not idle', async () => {
    ...
    // Register a process so closeStdin has an agent name
  ...
  it('cancelSession clears pending work and suppresses retries for an active session', async () => {
    let firstRun = true;
    ...
    queue.cancelSession('group1@g.us');
    resolveFirstProcess!();
    await vi.advanceTimersByTimeAsync(10);
